...
- **Today's Recap (direct local DB query)** — The `todays_recap` tool queries `~/.copilot/session-store.db` directly via `node:sqlite` and returns actual data + a presentation template. Falls back to returning raw SQL for the AI to execute manually if `node:sqlite` is unavailable. SQL does the heavy computation (engagement heuristics via keyword matching, context switch detection via `LAG` window functions); the AI just presents and narrates.
